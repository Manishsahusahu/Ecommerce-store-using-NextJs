...
const Navbar = ({ cart, addToCart, removeFromCart, clearCart, subTotal }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center md:flex-row md:justify-between py-2 bg-slate-50 sticky top-0">
      ...
      <div className="cart absolute right-2 md:relative flex space-x-2">
        <Link href="/login">
          <MdAccountBox className="text-xl md:text-3xl cursor-pointer" />
        </Link>
        <AiOutlineShoppingCart
          onClick={toggleCart}
      ...
      <div
        className={`z-100  h-[100vh] sideCart absolute top-0 right-0 bg-yellow-200 px-4 py-10 transform transition-transform ${translateCart}`}
      >
        ...
        <ol className="mt-5">
          {Object.keys(cart).length == 0 && (
            <div className="my-3 text-center font-normal">Cart is empty!</div>
          )}
        ...
        <div className="subtotal_checkout my-3">
          <span className="font-medium">Total: ₹</span>
          {subTotal}
        </div>
        <div className="cart__buttons flex space-x-1">
          <Link href={"/checkout"}>
